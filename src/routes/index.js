...
router.get('/', function(req, res, next) {
  ...
  var md = null;
  try {
    var filecontents = fs.readFileSync(path.join(settings.contentFolder, req.url.replace(/%20/g, ' '), 'index.md'), 'utf8');
...
router.get('/*.md', function(req, res, next) {
  var filepath = path.join(settings.contentFolder, req.url.replace(/%20/g, ' '))
